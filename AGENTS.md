...
- **进入本目录后第一步先读 `PROJECT.md`**：这是本系统的"总说明书"，汇总了云端实时数字、架构、数据模型、业务规则（含删除类任务红线）、可复用工具脚本索引、踩坑经验、已修复记录（216b032）与待老板拍板事项。旧《系统全量交接文档.md》《运维避坑与速查手册.md》《业务流程与待办联动·学员分层设计方案.md》已归档 `_archive/2026-09-08_legacy_docs/`，不再作为主入口。
- **云端探测/验证脚本一律复用 `tools/_lib.py` 公共库**（登录/请求/bootstrap 重试/查单个学员含订单），禁止再复制粘贴登录模板；新写脚本参考 `tools/_lib.py` 文件头。
- **删除类任务红线**：删除学员/订单/档案类操作，先出清单等老板拍板，不确定的放一边、排在所有其他任务之后执行；除非任务本身只针对删除。
...
云端当前包含 326 名学员档案（在读288/已结课32/待开课6，含 C- 前缀拆分档案；首次报名时间已于2026-09-08按最早订单/报名修正274人）、281 户家庭、347 条报名记录（与花名册Excel一致；首页看板：在班301人次/去重282人/46门班）、班级视图49行（46门在班+3行空班展示）、94 项排课矩阵与 579 行/579 单订单（2026-09-08删除重复173行，累计缴费口径314万；仅保留2026暑期+2026秋两季，更早全量历史底账见本地Excel）。**最新数字以 PROJECT.md 第一节为准，当次实时口径用 tools/_lib.py 拉取**。数据表设计如下：
...
| `enrollments` | 学员报读流水记录（347条，已去重） | `eid` (主键), `student_id`, `class_name`, `term`, `is_void` |
...
| `leaves` | 在线请假与退费消课台账（4条：第1讲请假缺课3人+金奕君事假；退费退班自动联动入台账） | `lid` (主键), `student_id`, `class_name`, `refund_amount` |
