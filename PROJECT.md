...
> 更新时间：2026-09-10（二期验收后临时变更单已提交并推送 `0e6a5b7`：家庭归属核对/催收反馈全链路下线、人次口径统一、请假退费类型拆分、待办保存体验优化；部署验证与云端 bootstrap 回归已通过。迁移 SQL 仍待老板在 Supabase SQL Editor 执行）。本文件取代旧《系统全量交接文档.md》《运维避坑与速查手册.md》《业务流程与待办联动·学员分层设计方案.md》，旧文档已归档 `deploy_zhujiao/_archive/2026-09-08_legacy_docs/`，不再作为主入口。
...
**线上验证（2026-09-10，部署 `0e6a5b7` 后）**：`tools/_verify_deploy.py` 已确认新版上线；bootstrap 200；`fb_collect` 存量从 5 条降为 0；`family_check` 为 0；待办 bootstrap 503 条仅剩 manual 332、leave_followup 7、weekly_followup 164；英文 family-review 泄漏风险 0；leaves 类型返回请假 7 / 退费退班 2。反馈第1讲总计仍为 305（含 6 条历史验收测试数据，非展示过滤问题）；真实业务上限 299 需在删除类测试数据清理获老板拍板后处理。
