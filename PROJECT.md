...
> 更新时间：2026-09-08（修复部署 216b032 之后）。本文件取代旧《系统全量交接文档.md》《运维避坑与速查手册.md》《业务流程与待办联动·学员分层设计方案.md》，旧文档已归档 `deploy_zhujiao/_archive/2026-09-08_legacy_docs/`，不再作为主入口。
...
## 八、已修复记录（commit 216b032 · 2026-09-08 上线验证）
...
| 事项 | 状态 | 说明 |
| :--- | :--- | :--- |
| **32 个 C- 前缀档案**（不在读但挂历史订单） | ⏸ **等老板拍板** | 明细已生成 `data/C-档案明细_老板核对.md`；方案 A 保持不动（推荐）/ B 隐藏归档 / C 合并订单后删除（需逐条确认金额归属）。**删除必须老板拍板后执行** |
...
| 二期设计清单 | ⏸ 未排期 | 新生5步待办链、周期待办、请假三分支自动折算、学员分层 S/A/B/C（见旧业务流程设计方案，已归档） |
...
- `deploy_zhujiao/` → 源码主目录（git）
  ...
  - `tools/` 全部脚本（`_*` 不进 git；`_lib.py` 公共库）
  - `data/` 老板可读产物（如 C-档案明细）与本地核对报告（敏感产物 gitignore）
  - `_archive/` 历史留档（gitignore）
- 根目录：`AGENTS.md`（工作规范）、《双人名学员档案_人工核对清单.md》（待拍板）、`课程知识库/`（本地175讲）、`教师反馈内容/`（反馈原件）、`讲义/`