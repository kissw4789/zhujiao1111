...
> 更新时间：2026-09-09（统一待办工作流已提交并推送，最新提交 `6536d31`；迁移 SQL 待老板在 Supabase SQL Editor 执行；云端验收在迁移执行并开启代理后补跑）。本文件取代旧《系统全量交接文档.md》《运维避坑与速查手册.md》《业务流程与待办联动·学员分层设计方案.md》，旧文档已归档 `deploy_zhujiao/_archive/2026-09-08_legacy_docs/`，不再作为主入口。
...
| 统一待办工作流改造（09-09 最新 PRD） | 🚧 **代码已上线，迁移待执行** | 提交 `6536d31` 已推送；五态/周周期/请假后续/反馈批次/调课转班/分级原子同步/统一导航/待办中心全部落地；待老板执行 `supabase/2026-09-09_统一待办工作流_一键迁移.sql` 并开启代理后补跑云端验收（详见 §8.4） |
